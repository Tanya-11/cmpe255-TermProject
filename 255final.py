...
Original file is located at
    https://colab.research.google.com/drive/1mfvNOiPiMrNu85O4tx_JZGTYGTvITfUZ
"""
...
"""Adding visualization to understand target feature phising"""
...
"""Looking for columns with non-unique values and dropping them"""
...
"""The next step is replacing all the -1's with NANs"""
...
# Visualizing Null values in each columns
...
"""#Imputing the data using Mean, Median and KNN Imputation
...
# rounding off domain_spf values to give better accuracy
...
1.   First on mean imputed data
2.   then on KNN Imputed Data

Analysis on Mean Imputed data
"""
...
"""Next step is to split the data to create test and train sets.
...
"""Performing Hyper-tunning to improve performance"""
...
Looking into Outliers and removing them
"""
...
"""Graphs to depict that outliers have been removed
...
"""Looking for the correlation between features and the target feature.
...
"""From the above analysis, we can conclude that the logistic regression model overfits"""
...
"""Performing the analysis on KNN Imputed Data"""
...
#Looking for outliers in RFECV (Important Features)
...
# Analyzing performance metrics for logistic regression
...
"""Here we apply Normalization to different columns
...
df_mm = pd.DataFrame(df_mm, columns=X_train_col_names)
...
sns.kdeplot(df_mm[X_train_col_names[0]],ax=ax1)
sns.kdeplot(df_mm[X_train_col_names[1]], ax=ax1)
sns.kdeplot(df_mm[X_train_col_names[2]], ax=ax1)
sns.kdeplot(df_mm[X_train_col_names[3]], ax=ax1)
sns.kdeplot(df_mm[X_train_col_names[4]], ax=ax1)
sns.kdeplot(df_mm[X_train_col_names[5]], ax=ax1)

"""RobustScaler"""

r_scaler  = preprocessing.RobustScaler()
df_mm = r_scaler.fit_transform(X_train)
...
df_mm = s_scaler.fit_transform(X_train)
...
df_n = n_scaler.fit_transform(X_train)

df_n = pd.DataFrame(X_train, columns=X_train_col_names)
...
"""**Conclusion**
Lets take a look at the conclusion we can draw from the analysis
*  The best accuracy for the KNN imputed data is while using RandomForestClassifier - 94.72%
*  The best accuracy for the mean imputed data is while using KNNClassifier - 95.34%.
*  The most relevant features correspond to attributes depending on URL and external services, according to both knn imputated data analysis and mean imputed data analysis.
